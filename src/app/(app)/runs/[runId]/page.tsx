...
export default function RunDetailsPage() {
  ...
  const handleFetchAndPreviewData = async () => {
    ...
    try {
        ...
        addLog(`Data Preview: Processed ${sampleForStorage.length} rows for storage.`); updateRunMutation.mutate({ id: runId, previewedDatasetSample: sampleForStorage, status: 'DataPreviewed', errorMessage: null, results: [] }); toast({ title: "Data Preview Ready", description: `${sampleForStorage.length} rows fetched and mapped.`});
  ...
  const simulateRunExecution = async () => {
    ...
    updateRunMutation.mutate({ id: runId, status: 'Processing', progress: 0, errorMessage: null, results: [] }); setSimulationLog([]); addLog("LLM Categorization process initialized."); let collectedResults: EvalRunResultItem[] = [];
    try {
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <Tabs defaultValue="results_table">
        ...
        <TabsContent value="breakdown"> {metricsBreakdownData.length === 0 && (!runDetails?.results || runDetails.results.length === 0) && ( <Card> <CardHeader> <CardTitle className="flex items-center"> <BarChartHorizontalBig className="mr-2 h-5 w-5 text-primary"/>Metrics Breakdown </CardTitle> </CardHeader> <CardContent> <p className="text-muted-foreground">No results available to generate breakdown.</p> </CardContent> </Card> )} {metricsBreakdownData.map(paramChart => ( <Card key={paramChart.parameterId} className="mb-6"> <CardHeader> <CardTitle className="flex items-center"> <BarChartHorizontalBig className="mr-2 h-5 w-5 text-primary"/> {paramChart.parameterName} </CardTitle> {runDetails.runType === 'GroundTruth' && paramChart.accuracy !== undefined && ( <CardDescription className="flex items-center mt-1"> <CheckCheck className="h-4 w-4 mr-1.5 text-green-600" /> Accuracy: {paramChart.accuracy.toFixed(1)}% {paramChart.totalCompared !== undefined && ` (${(paramChart.accuracy/100 * paramChart.totalCompared).toFixed(0)}/${paramChart.totalCompared} correct)`} </CardDescription> )} {runDetails.runType === 'Product' && ( <CardDescription className="flex items-center mt-1"> <Info className="h-4 w-4 mr-1.5 text-blue-600" /> Label distribution. </CardDescription> )} </CardHeader> <CardContent> {paramChart.data.length === 0 ? ( <p className="text-muted-foreground">No data recorded for this parameter.</p> ) : ( <ChartContainer config={{ count: { label: "Count" } }} className="w-full" style={{ height: `${Math.max(150, paramChart.data.length * 40 + 60)}px` }} > <ResponsiveContainer width="100%" height="100%"> <RechartsBarChartElement data={paramChart.data} layout="vertical" margin={{ right: 30, left: 70, top: 5, bottom: 20 }}> <CartesianGrid strokeDasharray="3 3" /> <XAxis type="number" allowDecimals={false} tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} /> <YAxis dataKey="labelName" type="category" width={120} tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} interval={0} /> <Tooltip content={<ChartTooltipContent />} cursor={{ fill: 'hsl(var(--muted))' }} /> <RechartsBar dataKey="count" fill="hsl(var(--chart-1))" radius={[0, 4, 4, 0]} barSize={20} /> </RechartsBarChartElement> </ResponsiveContainer> </ChartContainer> )} </CardContent> </Card> ))} {runDetails?.results && runDetails.results.length > 0 && metricsBreakdownData.length === 0 && ( <Card> <CardHeader> <CardTitle className="flex items-center"> <BarChartHorizontalBig className="mr-2 h-5 w-5 text-primary"/>Metrics Breakdown </CardTitle> </CardHeader> <CardContent> <p className="text-muted-foreground">Results are present, but no label counts could be generated.</p> </CardContent> </Card> )} </TabsContent>
      </Tabs>
