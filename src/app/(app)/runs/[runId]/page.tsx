...
export default function RunDetailsPage() {
  ...
  const hasMismatches = useMemo(() => { if (runDetails?.runType !== 'GroundTruth' || !runDetails.results || !evalParamDetailsForLLM) return false; return runDetails.results.some(item => evalParamDetailsForLLM.some(paramDetail => { const llmOutput = item.judgeLlmOutput[paramDetail.id]; const gtLabel = item.groundTruth ? item.groundTruth[paramDetail.id] : undefined; return gtLabel !== undefined && llmOutput && llmOutput.chosenLabel && !output?.error && String(llmOutput.chosenLabel).toLowerCase() !== String(gtLabel).toLowerCase(); })); }, [runDetails, evalParamDetailsForLLM]);
